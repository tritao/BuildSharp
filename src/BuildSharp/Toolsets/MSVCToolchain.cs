...
namespace BuildSharp.Toolsets
{
    public class MSVCToolchain : Toolchain
    {
        public MSVCToolchain()
            : base(ToolchainKind.MSVC)
        {
        }

        public override List<string> GetSystemIncludes()
        {
        ...
        static bool GetWindowsSDKDir(out string path)
        {
            var versions = new List<ToolchainVersion>();
        ...
        static bool GetVisualStudioDir(out string path)
        {
            ...
            path = versions.Last().Directory;
            path = path.Substring(0, path.LastIndexOf("\\Common7\\IDE",
